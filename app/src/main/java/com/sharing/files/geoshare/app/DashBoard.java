...
public class DashBoard extends AppCompatActivity implements GoogleApiClient.ConnectionCallbacks,GoogleApiClient.OnConnectionFailedListener,LocationListener {
                    ...
                    public void onDataChange(DataSnapshot snapshot) {
                        ...
                        for (DataSnapshot data : snapshot.getChildren()) {
                            ...
                            if (fkey.equals(key)) {
                                ...
                                int ra = Integer.parseInt(data.child("Radius").getValue().toString());
                                double cradius = distance(mLastLocation.getLatitude(),mLastLocation.getLongitude(),la,lo);
                                break;**/
                            } else {
                                flag = false;
                            }
    ...
    /**private double distance(double latitude_1, double longitude_1, double latitude_2, double longitude_2) {
        double earthRadius = 6378.137; // in miles, change to 6371 for kilometers
        double dLat = latitude_2*Math.PI/180-latitude_1*Math.PI/180;
        double dLng = longitude_2*Math.PI/180-longitude_1*Math.PI/180;
        double a = Math.sin(dLat/2)*Math.sin(dLat/2)+Math.cos(latitude_1*Math.PI/180)*Math.cos(latitude_2*Math.PI/180)*Math.sin(dLng/2)*Math.sin(dLng/2);
    ...
    private void uploadFile(final Uri allFilesUri) {
        ...
        storageReference.child(fileName).putFile(allFilesUri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                ...
                reference.child(fileName).setValue(taskSnapshot.getDownloadUrl().toString()).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                ...
                reference.child(fileName).child("Name").setValue(fileName);
                ...
                reference.child(fileName).child("URL").setValue(taskSnapshot.getDownloadUrl().toString());
            }
